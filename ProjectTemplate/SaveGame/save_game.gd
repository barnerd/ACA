...
const ENCRYPTION_KEY: String = "atveragegames"
const SAVE_GAME_FILE_NAME: String = "savegame.save"
...
static func delete_save() -> void:
	...
	DirAccess.remove_absolute("user://" + SAVE_GAME_FILE_NAME)


static func has_save() -> bool:
	return FileAccess.file_exists("user://" + SAVE_GAME_FILE_NAME)
...
static func save_game(tree: SceneTree):
	...
	print("Saving game to user://" + SAVE_GAME_FILE_NAME)
	
	var save_file: FileAccess = null
	
	if OS.is_debug_build():
		save_file = FileAccess.open("user://" + SAVE_GAME_FILE_NAME, FileAccess.WRITE)
	else:
		save_file = FileAccess.open_encrypted_with_pass("user://" + SAVE_GAME_FILE_NAME, FileAccess.WRITE, ENCRYPTION_KEY)
	
	for node in tree.get_nodes_in_group(SAVE_GROUP_NAME):
		var save_data = {}
...
static func load_game(tree: SceneTree) -> void:
	...
	print("Load game from user://" + SAVE_GAME_FILE_NAME)
	...
	if OS.is_debug_build():
		save_file = FileAccess.open("user://" + SAVE_GAME_FILE_NAME, FileAccess.READ)
	else:
		save_file = FileAccess.open_encrypted_with_pass("user://" + SAVE_GAME_FILE_NAME, FileAccess.READ, ENCRYPTION_KEY)
